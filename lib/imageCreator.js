...
exports.createFromMultiParameters = function(query, filename, fa, savingDir, callback) {
	fsName = "Segmentation/" + filename;
	faName = "FA/" + filename;
	var command = "/usr/bin/java -jar " + jarLocationMultiParameters + " '" + query + "' '" + fa + "' " + savingDir + " " + imgDir + " " + filename + " " + fsName + " " + faName;
	console.log("CREATOR: " + command);
	child = exec(command, function(err, result) {
		callback(result);
	});
