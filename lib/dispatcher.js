...
exports.dispatchGET = function(req, res) {
	...
	// Find the label when surface is clicked
		...
		this.createHTMLpage(files, savingDir, onlyStructure, res);
	}

	else if (path.match("complexSearch")) {
		var andColors = req.url.query.andColors;
		var orColors = req.url.query.andColors;
		var filename = req.url.query.filename;
		var onlyStructure = req.url.query.onlyStructure;
		console.log("SERVER: multiLabels search with input: " + andColors + " " + orColors + " " + filename + " " + onlyStructure);
...
exports.createHTMLpage = function(files, savingDir, onlyStructure, res) {
